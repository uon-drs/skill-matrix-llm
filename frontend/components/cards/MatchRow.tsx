 ...
 * Displays avatar, researcher name with match-reason summary, a MatchMeter
 * bar, and a right-facing arrow. Background transitions to `--bg-2` on hover.
 * @param researcher - Researcher data object
 * @param onClick - Click handler
 */
export function MatchRow({ researcher, onClick, className }: MatchRowProps) {
  return (
    <div
      onClick={onClick}
      className={cn(
    ...
    >
      ...
      <div>
        ...
        <div className="font-mono text-[11.5px] text-ink-muted mt-[2px]">
          {researcher.discipline} · overlap on{" "}
          {researcher.matchReasons.slice(0, 2).join(", ")}
        </div>
      </div>
