namespace TemplateApp.Api.Services.EmailSender;
...
public class SmtpEmailSender : IEmailSender
{
    ...
    ILogger<SmtpEmailSender> logger)
  {
    ...
    if (!IsSmtpOptionsAvailable())
    {
      LogError("SMTP values missing or incomplete");
    }
  }
  ...
  {
    toAddresses =
      toAddresses.Where(x => !_config.ExcludedEmailAddresses.Contains(x.Address))
        .ToList(); // filter out blocked email addresses
  ...
  private bool IsSmtpOptionsAvailable()
  {
    ...
    if (_config.SmtpPort == 0 || _config.SmtpSecureSocketEnum is > 4 or < 1)
    {
      return false;
    }
    ...
    if (_config.UseOAuth)
    {
      string[] oauthOptions =
      [
        _config.OAuthTokenEndpoint, _config.OAuthClientId, _config.OAuthScope
      ];

      if (oauthOptions.Any(string.IsNullOrEmpty))
      {
        return false;
      }

      return !string.IsNullOrEmpty(_config.OAuthClientSecret);
    }

    string[] basicAuthOptions =
    [
      _config.SmtpUsername, _config.SmtpPassword
    ];
    return !basicAuthOptions.Any(string.IsNullOrEmpty);
  }
  ...
  private async Task<SmtpClient> ConnectAndAuthenticate()
  {
    ...
    try
    {
      ...
      if (_config.UseOAuth)
      {
        var response = await _config.OAuthTokenEndpoint
          .PostUrlEncodedAsync(new
          {
            client_id = _config.OAuthClientId,
            client_secret = _config.OAuthClientSecret,
            scope = _config.OAuthScope,
        ...
        var oauth2 = new SaslMechanismOAuth2(
          _config.FromAddress,
          response.AccessToken ?? throw new InvalidOperationException("Failed to acquire OAuth token"));
